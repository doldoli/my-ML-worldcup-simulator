...
class MLPClassifier(BaseEstimator, ClassifierMixin):
    ...
    def _relu_derivative(self, Z: np.ndarray) -> np.ndarray:
        # Backpropagation 시 사용할 ReLU의 미분값. 0보다 크면 1, 아니면 0 반환. 
        # Boolean 배열 -> float로 변환
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> None:
        ...
        if n_classes == 2:
            ...
            # MLP는 행렬 연산이기에 예측값이 (n_samples, 1)의 2차원 행렬로 출력됨
            # 오차 계산시 차원을 맞춰주기 위해 정답 y도 열이 1개인 2차원 행렬로 차원 변환해줌.
        ...
        # self.hidden_layer_sizes 는 튜플 형태기에 type casting.
        ...
        for i in range(len(layer_sizes) - 1):
            # 가중치를 0 혹은 0이 아니더라도 같은값으로 초기화하면 모든 뉴런이 동일한 Gradient를 받아 뉴런들이 모두 동일하게 업데이트됨 (학습 불가)
            # 이를 방지하기 위해 난수(표준정규분포 - 음수도 포함시켜 학습의 유연성 높임)를 사용하여 초기 가중치를 뉴런마다 조금씩 다르게 부여함
            # 또한 초기 가중치 값이 너무 크면 활성화 함수에 따라 기울기 소실이 발생할 수 있기에 0.01을 곱해 분산을 줄여줌
            ...
            # 리스트에 Numpy 배열 하나씩 추가. 반복문 종료 후 self.weights_[i]에는 i번째 층의 가중치 행렬이 저장. 순전파 연산 시 인덱스로 접근 가능
    ...
    def _forward(self, X: np.ndarray) -> tuple[list[np.ndarray], list[np.ndarray]]:
        ...
        # 역전파 단계에서 가중치 행렬을 업데이트하기 위한 Gradient를 구할 때 들어온 입력값(x_l)이 필요하기에 이를 기록해두어야 함. 
        ...
        # 따라서 활성화 함수를 통과하기 전 가중합 행렬 Z를 층별로 기록해두어야 함
        Zs = [] 
        ...
        for l in range(L - 1):
            # self.weights_[l]은 l층 노드와 l+1층 노드를 연결하는 가중치

            # A (입력 신호): (배치 데이터 수, l층 노드 수) l층의 출력값
            # W (가중치): (l층 노드 수, l+1층 노드 수) -> self.weights_[i]
            # b (편향): (l+1층 노드 수, ) -> self.biases_[i]
            # Z (가중합): (배치 데이터 수, l+1층 노드 수) 가중치를 통과해 계산된 l+1층의 가중합
    ...
    def _backward(self, Y_batch: np.ndarray, activations: list[np.ndarray], Zs:list[np.ndarray]) -> None:
        ...
        # 각 층의 가중치(W)와 편향(b)의 Gradient를 담을 빈 리스트 생성
        # np.zeros_like: 입력된 배열과 같은 차원(Shape)을 가지지만, 값은 0으로 채워진 배열 생성
        # 가중치 행렬 수(은닉층 + 1) 만큼
        ...
        for l in reversed(range(L)):
            # self.weights_[l]: l층 노드와 l+1층 노드를 연결하는 가중치 행렬
            # activations[l]: 가중치 행렬을 통과하기 전 l층 노드의 출력값 (입력신호)
            # Zs[l-1]: l층 노드의 활성화 미분값을 구할 때 필요한 l층의 가중합

            # A_prev: (배치 데이터 수, l층 노드 수) 가중치 행렬을 통과하기 전 l층 노드의 출력값 (입력신호, x_l)
            A_prev = activations[l]

            # Gradient
            # dws[l]: (l층 노드 수, l+1층 노드 수) 가중치 행렬의 Gradient
            # l층의 출력값(A_prev)과 l+1층의 오차(dZ)를 행렬곱하여 m개 데이터에 대해 평균 냄 (eta_(l+1) * x_l)
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        if self.multi_class_ == 'binary':
            ...
            # 2차원 행렬 (n_samples, 1)을 1차원 배열 (n_samples,)로 바꾸기 위해 .flatten() 사용
        ...
        # self.classes_에는 fit()에서 저장해둔 고유 클래스 이름이 들어있음
