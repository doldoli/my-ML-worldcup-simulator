...
def check_array(X: np.ndarray | list) -> np.ndarray:
    ...
    try:
        ...
        # 행렬 연산의 수치적 안정성을 위해 float64로 캐스팅
    except ValueError:
        raise ValueError("입력 데이터 X는 숫자형(float or int)으로 변환할 수 있어야 합니다.")
    
    if X.ndim == 1:
        # 1차원 배열(n_samples,) 입력 시 2차원 (n_samples, 1)로 차원 확장
        X = X.reshape(-1, 1)
    elif X.ndim > 2:
...
def check_X_y(X: np.ndarray | list, y: np.ndarray | list) -> tuple[np.ndarray, np.ndarray]:
    ...
    if y.ndim > 1:
        if y.shape[1] == 1:
            # (n_samples, 1) 형태인 [[a], [b], ...] 구조의 2차원 Target 배열을 (n_samples, ) 1차원으로 변환
            y = np.ravel(y)
        else:
